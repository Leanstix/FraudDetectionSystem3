...
class EntityResolutionAgent(BaseAgent):
    ...
    def run(
        ...
        with_comms = self.resolver.attach_communication_context(with_locations, sms_df, mails_df)
        enriched = self.resolver.attach_audio_context(with_comms, audio_df)
        ...
        diagnostics = {
            ...
            "audio_link_rate": float((enriched["audio_link_count"] > 0).mean()) if len(enriched) else 0.0,
        }

        profiles = self.resolver.build_entity_profiles(enriched, users_df, locations_df, audio_df)
        return {
